   ...
   * '업무_데이터' 시트에서 중복을 제외한 '유형' 목록을 가져옵니다.
    */
     ...
     * '유형' 이름으로 '업무_데이터'를 필터링합니다.
      * @param {string} typeName - 필터링할 유형의 이름
       */
       function filterDataByType(typeName) {
          try {
                ...
                return { error: `[filterDataByType] ${e.toString()}` };
          }
       }
        ...
        * 키워드로 데이터를 검색합니다.
         * @param {string} searchType - '업무 검색' 또는 '이벤트 검색'
          * @param {string} searchTerm - 검색어
           * @param {string} searchField - '업무 검색' 시 검색할 필드
            */
            function searchData(searchType, searchTerm, searchField) {
                  try {
                        ...
                        return { error: `[searchData] ${e.toString()}` };
                  }
            }

            /**
             * (헬퍼 함수) 데이터 행 하나를 프론트엔드로 보낼 형식으로 가공합니다.
              * @param {Array} rowValues - getValues()로 얻은 단일 행 데이터
               * @param {Array} richTextRow - getRichTextValues()로 얻은 단일 행 데이터
                * @param {string} searchType - '업무 검색' 또는 '이벤트 검색'
                 * @returns {Array} - 가공된 결과 행
                  */
                                  ...
                                  for (let k = 0; k < 8; k++) { // A-H열
                                        let textValue = rowValues[k];
                                              if (dateIndices.includes(k) && textValue) {
                                                    ...
                                                    resultRow.push({ text: textValue, url: richTextRow[k] ? richTextRow[k].getLinkUrl() : null });
                                  }
                          }
                            return resultRow;
                  }
